...
from aiogram.types import CallbackQuery, Message, ReplyKeyboardMarkup, KeyboardButton, ReplyKeyboardRemove, InputFile
...
def init_db():
    ...
    # Таблица товаров (добавлены поля для цен и изображений)
...
def add_product(name, description, supplier_price, category_id, supplier_id, quantity=0, approved=False, image_path=None):
    ...
    cur.execute("""
        INSERT INTO products (name, description, supplier_price, final_price, category_id, supplier_id, approved, quantity, in_stock, image_path) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (name, description, supplier_price, final_price, category_id, supplier_id, approved, quantity, in_stock, image_path))
...
def update_product(product_id, name=None, description=None, supplier_price=None, category_id=None, quantity=None, in_stock=None, image_path=None):
...
def reject_product(product_id):
    ...
    cur.execute("SELECT image_path FROM products WHERE id = ?", (product_id,))
    result = cur.fetchone()
...
def get_supplier_products(supplier_id):
    ...
    cur.execute("""
        SELECT id, name, supplier_price, approved, in_stock, quantity, image_path 
        FROM products 
        WHERE supplier_id = ?
    """, (supplier_id,))
    products = cur.fetchall()
...
def get_visible_products(category_id=None, offset=0, limit=10):
    ...
    if category_id:
        cur.execute("""
            SELECT id, name, final_price, in_stock, quantity, image_path 
            FROM products 
            WHERE approved = 1 
              AND status = 'active'
              AND in_stock = 1
...
async def notify_admins_about_product(product_id):
    try:
        ...
        # Правильная индексация полей (image_path теперь на 11 позиции)
        (_, name, description, supplier_price, final_price, 
         category_id, supplier_id, approved, status, in_stock, 
         quantity, image_path, created_at) = product
        
        supplier = get_user_by_id(supplier_id)
        
        if not supplier:
            logger.error(f"Поставщик {supplier_id} не найден")
            return
        ...
        logger.error(f"Ошибка при отправке уведомления о товаре: {e}")
    finally:
        await bot.session.close()
...
async def show_main_category(callback: CallbackQuery):
    ...
    try:
        ...
        if not subcategories:
            # Если нет подкатегории, сразу показываем товары
...
async def show_category_products(callback: CallbackQuery, category_id: int, page: int = 0, limit: int = 10):
    ...
    if not products:
        ...
        await callback.message.edit_text(
            "😔 В этой категории пока нет товары",
            reply_markup=builder.as_markup()
        )
        return
    
    for product in products:
        product_id, name, price, in_stock, quantity, image_path = product
...
async def show_product(callback: CallbackQuery):
    try:
        ...
        # Отправляем фото товара или текст
        if image_path and os.path.exists(image_path):
            with open(image_path, 'rb') as photo:
                await callback.message.answer_photo(
                    photo,
                    caption=f"<b>{name}</b>\n\n{description}\n\n💰 Цена: <b>{price} руб.</b>\n📦 Наличие: <b>{stock_text}</b>\n📊 Количество: <b>{quantity} шт.</b>",
                    parse_mode="HTML",
...
async def process_quantity(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("📸 Отправьте фото товара (или нажмите /skip чтобы пропустить):")
        await state.set_state(ProductForm.image)
        
    except ValueError:
        await message.answer("❌ Неверный формат количества. Введите целое число:")

@supplier_router.message(Command("skip"), ProductForm.image)
...
@supplier_router.message(F.photo, ProductForm.image)
async def process_image(message: types.Message, state: FSMContext):
    try:
        ...
        file_ext = file.file_path.split('.')[-1] if file.file_path else 'jpg'
        ...
        await message.answer("❌ Ошибка при загрузке изображения. Попробуйте еще раз или пропустите (/skip):")
...
async def handle_products(message: types.Message):
    try:
        ...
        builder = InlineKeyboardBuilder()
        for product in products:
            product_id, name, supplier_price, approved, in_stock, quantity, image_path = product
...
async def choose_edit_field(callback: CallbackQuery, state: FSMContext):
    ...
    builder.add(types.InlineKeyboardButton(text="Изображение", callback_data="edit_image"))
...
async def process_edit_description(message: types.Message, state: FSMContext):
    ...
    await notify_admins_about_product(product_id)
    await message.answer("✅ Описание обновлено! Товар снова отправлен на модeraцию.")
    await state.clear()
...
@supplier_router.callback_query(F.data == "edit_image")
async def edit_product_image(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer("📸 Отправьте новое фото товара (или /skip чтобы удалить текущее):")
    await state.set_state(EditProduct.image)
    await callback.answer()

@supplier_router.message(Command("skip"), EditProduct.image)
async def skip_edit_image(message: types.Message, state: FSMContext):
    data = await state.get_data()
    product_id = data.get('product_id')
    
    # Удаляем текущее изображение
    product = get_product_details(product_id)
    if product and product[11] and os.path.exists(product[11]):
        try:
            os.remove(product[11])
        except:
            pass
    
    # Обновляем товар без изображения
    update_product(product_id, image_path="")
    
    await message.answer("✅ Изображение удалено! Товар снова отправлен на модерацию.")
    await state.clear()
    await notify_admins_about_product(product_id)

@supplier_router.message(F.photo, EditProduct.image)
async def process_edit_image(message: types.Message, state: FSMContext):
    try:
        ...
        update_product(product_id, image_path=file_path)
        
        await message.answer("✅ Изображение обновлено! Товар снова отправлен на модерацию.")
        await state.clear()
        await notify_admins_about_product(product_id)
        ...
        for product in products:
            ...
            if image_path and os.path.exists(image_path):
                try:
                    ...
                    logger.error(f"Ошибка при отправке фото товара {product_id}: {e}")
                    await message.answer(
                        text,
                        parse_mode="HTML",
                        reply_markup=builder.as_markup()
